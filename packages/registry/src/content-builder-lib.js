/**
 * Pure MDX content-entry builders and field normalizers.
 *
 * This module holds the deterministic layer that turns MDX frontmatter/body
 * into registry entry objects (download trust, provenance, platform
 * compatibility, and section metadata). Nothing here touches the network —
 * given the same inputs the output is byte-stable.
 *
 * The public package surface (`content-builder.js` /
 * `@heyclaude/registry/content-builder`) re-exports everything below so
...
export function normalizeDateTimeField(value) {
...
export function buildProvenanceFields(data = {}) {
  return {
...
export function buildDefaultSkillPlatformCompatibility(data, inferred) {
  const verifiedAt =
...
export function normalizePlatformCompatibility(value, data, inferred) {
