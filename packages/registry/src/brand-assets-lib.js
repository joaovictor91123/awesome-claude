/**
 * Pure brand-asset resolution helpers.
 *
 * This module holds the deterministic domain normalization, known-brand
 * detection, Brandfetch/proxy URL construction, and metadata sanitization
 * layer used across registry entry assembly. Nothing here touches the network
 * — given the same inputs the output is byte-stable.
 *
 * The public package surface (`brand-assets.js` /
...
export function textContainsAlias(text, alias) {
...
export function knownBrandTextCandidates(data = {}) {
  const values = [data.brandName, data.title];
  if (Array.isArray(data.tags)) values.push(...data.tags);
  return values.map(clean).filter(Boolean);
}

export function knownBrandMatchesDomain(data = {}, domain = "") {
