...
installable: true
installCommand: >-
  mkdir -p .claude/hooks && touch .claude/hooks/real-time-activity-tracker.sh &&
  chmod +x .claude/hooks/real-time-activity-tracker.sh
usageSnippet: >-
  mkdir -p .claude/hooks && touch .claude/hooks/real-time-activity-tracker.sh &&
  chmod +x .claude/hooks/real-time-activity-tracker.sh
copySnippet: |-
...
Current session*id uses timestamp+PID but PID can conflict. Add random suffix: "$(date +%Y%m%d*%H%M%S)_$$_$RANDOM" or use UUID: "$(uuidgen 2>/dev/null || echo $RANDOM$RANDOM)". Verify session ID generation. Test with various session scenarios.
